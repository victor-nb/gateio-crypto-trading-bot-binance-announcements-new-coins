...
from auth.gateio_auth import load_gateio_creds
...
def get_last_price(base, quote, return_price_only):
    ...
    logger.info(
        f"GET PRICE: {t.currency_pair} | last={t.last} | change%={t.change_percentage} |"
        f" lowest_ask={t.lowest_ask} | highest_bid={t.highest_bid} | base_volue={t.base_volume} "
        f"| quote_volume={t.quote_volume}"
    )
    return t
...
def place_order(base, quote, amount, side, last_price):
    ...
    try:
        ...
        t = order
        logger.info(
            f"PLACE ORDER: {t.side} | {t.id} | {t.account} | {t.type} | {t.currency_pair} |"
            f" {t.status} | amount={t.amount} | price={t.price} | left={t.left} | filled_total="
            f"{t.filled_total} | fill_price={t.fill_price} | fee={t.fee} {t.fee_currency}"
        )
    except Exception as e:
        logger.error(e)
        raise
