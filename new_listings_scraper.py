...
def search_and_update():
    ...
    while not globals.stop_threads:
        ...
        try:
            ...
            logger.info(
                f"Checking for coin announcements every "
                f"{str(sleep_time)} seconds (in a separate thread)"
            )
...
def get_all_currencies(single=False):
    ...
    while not globals.stop_threads:
        ...
        with open("currencies.json", "w") as f:
            ...
            logger.info(
                "List of gate io currencies saved to currencies.json. Waiting 5 "
