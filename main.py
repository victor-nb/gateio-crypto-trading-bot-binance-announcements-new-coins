    ...
    try:
        while True:
            ...
            if len(order) > 0:
                for coin in list(order):
                    ...
                    if float(order[coin]["_tp"]) == 0:
                        ...
                        logger.Info(
                            f"Order is initialized but not ready. Continuing. | Status={st}"
                        )
                        continue
                    ...
                    logger.debug(
                        f"Data for sell: {coin=} | {stored_price=} | {coin_tp=} | {coin_sl=}"
                        f" | {volume=} | {symbol=} "
                    )
                    ...
                    logger.info(
                        f'{symbol=}-{last_price=}\t[STOP: ${"{:,.5f}".format(stop_loss_price)}'
                        f' or {"{:,.2f}".format(coin_sl)}%]\t[TOP: '
                        f'${"{:,.5f}".format(top_position_price)} or {"{:,.2f}".format(coin_tp)}%'
                        f']\t[BUY: ${"{:,.5f}".format(stored_price)} (+/-): '
                        f'{"{:,.2f}".format(((float(last_price) - stored_price) / stored_price) * 100)}%]'
                    )

                    # update stop loss and take profit values if threshold is reached
                    if (
                        float(last_price) > stored_price + (stored_price * coin_tp / 100)
                        and enable_tsl
                    ):
                        ...
                        logger.info(
                            f'updated tp: {round(new_tp, 3)}% / ${"{:,.3f}".format(new_top_position_price)}'
                        )
                        logger.info(
                            f'updated sl: {round(new_sl, 3)}% / ${"{:,.3f}".format(new_stop_loss_price)}'
                        )
                    ...
                    # close trade if tsl is reached or trail option is not enabled
                        ...
                        try:
                            ...
                            logger.info(
                                f"starting sell place_order with :{symbol} | {pairing} | {volume} | "
                                f"{sell_volume_adjusted} | {fees} | "
                                f"{float(sell_volume_adjusted)*float(last_price)} | side=sell | "
                                f"last={last_price}"
                            )
                            ...
                            if not test_mode:
                                ...
                                if sell._status != "closed":
                                    ...
                                    if float(sell._left) > 0 and float(sell._amount) > float(
                                        sell._left
                                    ):
                                        ...
                                        order[coin][
                                            "_fee"
                                        ] = f"{fees - (float(sell._fee) / float(sell._price))}"
                                        ...
                                        logger.info(
                                            ...
                                            "Adjusted order _amount and _fee to perform sell of "
                                            "remaining balance"
                                        )
                                    ...
                                    continue
                            sell_price = round(
                                (float(last_price) - stored_price) * float(volume), 3
                            )
                            profit = round(
                                (float(last_price) - stored_price) / float(stored_price) * 100, 3
                            )
                            logger.info(f"sold {coin} with {sell_price} profit | {profit}% PNL")
                        ...
                        # store sold trades data
                            ...
                            if not test_mode:
                                ...
                                profit = (float(last_price) - stored_price) / stored_price * 100
                                sold_coins[coin] = {
                                    "symbol": coin,
                                    "price": last_price,
                                    "volume": volume,
            ...
            if (
                ...
                and announcement_coin not in sold_coins
                and announcement_coin not in old_coins
            ):
                ...
                if supported_currencies:
                    if announcement_coin in supported_currencies:
                        ...
                        logger.info(
                            f"starting buy place_order with : {announcement_coin=} | {pairing=} | {volume=} |"
                        ...
                        try:
                            ...
                            # place a live order if False
                                ...
                                order[announcement_coin] = place_order(
                                    announcement_coin, pairing, volume, "buy", price
                                )
                            ...
                            message = (
                                f"Order created on {announcement_coin} at a price of {price} each. "
                                f" {order_status=}"
                            )
                            logger.info(message)

                            if order_status == "closed":
                                order[announcement_coin]["_amount_filled"] = order[
                                    announcement_coin
                                ]["_amount"]
                                session[announcement_coin]["total_volume"] = session[
                                    announcement_coin
                                ]["total_volume"] + (
                                    float(order[announcement_coin]["_amount"])
                                    * float(order[announcement_coin]["_price"])
                                )
                                session[announcement_coin]["total_amount"] = session[
                                    announcement_coin
                                ]["total_amount"] + float(order[announcement_coin]["_amount"])
                                session[announcement_coin]["total_fees"] = session[
                                    announcement_coin
                                ]["total_fees"] + float(order[announcement_coin]["_fee"])
                                session[announcement_coin]["orders"].append(
                                    copy.deepcopy(order[announcement_coin])
                                )
                                ...
                                if (
                                    ...
                                    > float(order[announcement_coin]["_left"])
                                    and float(order[announcement_coin]["_left"]) > 0
                                ):
                                    # partial order. Change qty and fee_total in order
                                    # and finish any remaining balance
                                    partial_amount = float(
                                        order[announcement_coin]["_amount"]
                                    ) - float(order[announcement_coin]["_left"])
                                    ...
                                    session[announcement_coin]["total_amount"] = (
                                        session[announcement_coin]["total_amount"] + partial_amount
                                    )
                                    session[announcement_coin]["total_fees"] = (
                                    ...
                                    logger.info(
                                        f"Parial fill order detected.  {order_status=} | {partial_amount=} "
                                        f"out of {amount=} | {partial_fee=} | {price=}"
                                    )

                                # order not filled, try again.
                                logger.info(
                                    f"clearing order with a status of {order_status}.  Waiting for "
                                    "'closed' status"
                                )
                        ...
                        logger.debug(
                            "Removed new_listing.json due to coin not being " "listed on gate io"
                        )
                else:
                    logger.error("supported_currencies is not initialized")
            else:
                logger.info(
                    "No coins announced, or coin has already been bought/sold. Checking more frequently in "
                    "case TP and SL need updating"
                )
